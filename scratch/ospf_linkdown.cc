...
void PeriodicCheckNeighbor() {
  CheckNeighbor();
  Simulator::Schedule (Seconds (0.1), &PeriodicCheckNeighbor);
}
...
/*	if(argc == 4){
  ...
  int32_t ipv4ifIndex = downInterface1;
  Simulator::Schedule (Seconds (0), &initLSAs);
  //Simulator::Schedule(Seconds(1), &PeriodicCheckNeighbor);
